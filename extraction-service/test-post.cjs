...
(async () => {
  ...
  const reactions = await page.evaluate(() => {
    ...
    for (const el of all) {
      ...
      if (label.match(/\d/) && (label.toLowerCase().includes("reaction") || label.toLowerCase().includes("like") || label.includes("طھظپط§ط¹ظ„") || label.includes("ط¥ط¹ط¬ط§ط¨") || label.includes("ط£ط¹ط¬ط¨"))) {
  ...
  if (reactions.length > 0) {
    ...
    await page.evaluate(() => {
      ...
      for (const el of all) {
        const label = el.getAttribute("aria-label") || "";
        if (label.match(/\d/) && (label.toLowerCase().includes("reaction") || label.includes("طھظپط§ط¹ظ„") || label.includes("ط¥ط¹ط¬ط§ط¨") || label.includes("ط£ط¹ط¬ط¨"))) {
          el.click();
          return;
        }
      }
