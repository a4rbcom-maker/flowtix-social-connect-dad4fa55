...
export class IgPostUsersExtractor extends IgBaseExtractor {
  ...
  async extract(): Promise<{ extracted: number; nextCursor?: string; done: boolean; authState: AuthState }> {
    ...
    // 1) Load the post; capture GraphQL comment/like pages while it renders.
    const client = new IgMediaClient();
    const first = await client.captureFirstPage(this.page, `${config.igBaseUrl}/p/${shortcode}/`, { scroll: 3 });
    let got = 0;
    if (first) for (const u of first.users) if (add(u)) { got++; engine.addResults(1); }

    // 2) DOM harvest of rendered comments (always works when visible).
    ...
    // 3) Engagers: also open the likers dialog and read its rows.
    ...
    // 4) Keep scrolling for long comment threads (page self-paginates).
    let stale = 0;
    while (
  ...
  private async openLikersAndCollect(): Promise<{ username: string; fullName?: string; avatar?: string }[]> {
    const clicked = await this.page
      .evaluate(() => {
        ...
        for (const el of cands) {
          ...
          if (/^[\d,.KkMm]+\s*(likes?|إعجاب)/i.test(txt)) { el.click(); return true; }
        }
        return false;
      })
      .catch(() => false);
    ...
    // scroll inside the dialog a few times to load more liker rows
      ...
      const rows = await this.page
        .evaluate(() => {
          ...
          for (const a of dialog.querySelectorAll('a[href^="/"]')) {
            ...
            if (!m || ["p", "reel", "explore", "accounts"].includes(m[1])) continue;
