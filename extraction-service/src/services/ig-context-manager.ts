...
import { parseProxyUrl } from "./context-manager.js";
...
class IgContextManager {
  ...
  async createContext(sessionId: string, cookies: CookieEntry[], proxy?: ProxyConfig | null): Promise<{ context: BrowserContext; page: import("playwright").Page; contextId: string }> {
    ...
    log.info("IgContextManager", `session ${sessionId.slice(0, 8)}: cookies injected = ${cookies.length} cookies (sessionid=${hasSessionId}, ds_user_id=${hasDsUserId})${proxy ? `, proxy=${proxy.label || proxy.url.split("@").pop() || "yes"}` : ""}`);
