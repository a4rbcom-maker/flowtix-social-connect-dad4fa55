...
async function proof() {
  ...
  const cookies = (Array.isArray(raw) ? raw : []).map((c: any) => ({ name: c.name, value: String(c.value), domain: c.domain || ".facebook.com", path: "/", expires: -1, httpOnly: false, secure: true, sameSite: "Lax" }));
