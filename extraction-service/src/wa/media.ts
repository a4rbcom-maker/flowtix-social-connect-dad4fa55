...
export const mediaService = {
  async downloadAndStore(workspaceId: string, sessionId: string, waMessageId: string, streamSupplier: () => Promise<Buffer>, mime: string): Promise<string | null> {
    try {
      const buf = await streamSupplier();
      const ext = (mime.split("/")[1] || "bin").split("+")[0];
      const key = `${workspaceId}/${sessionId}/${waMessageId}.${ext}`;
