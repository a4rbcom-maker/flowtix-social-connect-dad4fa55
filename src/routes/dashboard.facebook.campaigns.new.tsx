    ...
    <DashboardLayout title={t.title}>
      ...
      {confirmOpen && (
        <div
          ...
          onClick={() => !savingAndStart && setConfirmOpen(false)}
        >
          <div
            dir={dir}
          ...
          >
            <div className="flex items-start justify-between gap-3 border-b border-border px-5 py-4">
              <div>
                <h3 className="text-base font-bold text-foreground">{t.confirmTitle}</h3>
                <p className="mt-1 text-xs text-muted-foreground">{t.confirmDesc}</p>
              </div>
              <button
                onClick={() => !savingAndStart && setConfirmOpen(false)}
            ...
            <div className="px-5 py-4 space-y-3">
              <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
                ...
                {t.confirmCount(selectedTargets.size)}
              </div>
              ...
              <ul className="max-h-64 overflow-y-auto divide-y divide-border rounded-lg border border-border bg-background">
                {groups
                  ...
                  .map((g) => {
                    const shortId =
                      g.id.length > 10 ? `${g.id.slice(0, 4)}…${g.id.slice(-4)}` : g.id;
                    return (
                      <li
                        key={g.id}
            ...
            <div className="flex items-center justify-end gap-2 border-t border-border px-5 py-3">
              <button
                ...
                disabled={savingAndStart}
                className="rounded-lg border border-border bg-background px-3 py-2 text-xs font-semibold hover:bg-accent disabled:opacity-50"
              >
                {t.confirmCancel}
              </button>
              <button
