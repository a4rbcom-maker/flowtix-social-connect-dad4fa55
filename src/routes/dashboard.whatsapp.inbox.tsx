...
async function fetchInboxConversations(
  userId: string,
  opts?: { beforeTs?: string | null; limit?: number; sessionId?: string | null },
  ...
  let q = supabase
    ...
    .limit(limit);
  // Session scoping: when a session is currently paired, only show its chats.
  // Previous numbers' history stays in the DB but disappears from the inbox.
  if (opts?.sessionId) {
