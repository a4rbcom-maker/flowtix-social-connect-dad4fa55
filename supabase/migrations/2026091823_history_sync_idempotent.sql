-- History sync idempotency: messaging-history.set chunks can replay the same
-- message ids (re-link / overlapping chunks). Plain INSERT errors with 23505 on
-- every duplicate and the row is lost. ON CONFLICT DO NOTHING makes history
-- import safe to re-run; returns null uuid on duplicates.
