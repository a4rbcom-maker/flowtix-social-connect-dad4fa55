...
if [ -n "${STAGING_PATH:-}" ]; then
  ...
  echo "=== Syncing staged bundle into DEPLOY_PATH ==="
  if command -v sudo >/dev/null 2>&1 && sudo -n true 2>/dev/null; then
    sudo mkdir -p "$DEPLOY_PATH" || fail_deploy "deploy-path-create-failed"
    sudo rsync -a --delete \
      --exclude='.env' \
      --exclude='.user.ini' \
      --exclude='.htaccess' \
    ...
    mkdir -p "$DEPLOY_PATH" || fail_deploy "deploy-path-create-failed"
    rsync -a --delete \
      --exclude='.env' \
      --exclude='.user.ini' \
      --exclude='.htaccess' \
...
if [ -f .env ]; then
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    value="${line#*=}"
    case "$key" in
      SUPABASE_URL|SUPABASE_PUBLISHABLE_KEY|SUPABASE_SERVICE_ROLE_KEY|VITE_SUPABASE_URL|VITE_SUPABASE_PUBLISHABLE_KEY|BOT_ENCRYPTION_KEY|BOT_WORKER_SECRET|WA_BRIDGE_URL|WA_BRIDGE_API_KEY|WA_BRIDGE_WEBHOOK_SECRET|WA_PUBLIC_WEBHOOK_URL|FB_APP_SECRET|FB_WEBHOOK_VERIFY_TOKEN|FLOWTIX_ALERT_WEBHOOK_URL|ALERT_WEBHOOK_URL|SSR_ALERT_WEBHOOK_URL|ALERT_THROTTLE_MS)
